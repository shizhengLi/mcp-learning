...
export class TypeScriptAnalyzer extends BaseLanguageAnalyzer {
  ...
  protected analyzeCodeStructure(code: string): {
    ...
    const exports = this.extractImports(
      code,
      /export\s+(?:default\s+)?(?:function|const|let|class|interface)\s+(\w+)/g
    )
  ...
  protected generateLanguageSpecificSuggestions(
    ...
    const functionLines = lines.filter(
      line => line.trim().match(/^function\s+\w+/) && line.length < 50
    )
    ...
    const nullCheckCount = lines.filter(
      line => line.includes('!== null') || line.includes('!== undefined')
    ).length
