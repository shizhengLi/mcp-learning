...
export class QualityMetricsCalculator {
  ...
  async calculateQualityMetrics(
    ...
    const qualityTrend = this.calculateQualityTrend(
      overallScore,
      previousMetrics?.overallQualityScore
    )
  ...
  private calculateMaintainabilityIndex(baseMetrics: CodeMetrics, halsteadMetrics: any): number {
    ...
    let mi =
      171 - 5.2 * Math.log(halsteadVolume) - 0.23 * cyclomaticComplexity - 16.2 * Math.log(avgLoc)
  ...
  private countFunctions(code: string, language: string): number {
    ...
    const pattern =
      functionPatterns[language as keyof typeof functionPatterns] || /\bfunction\s+\w+\b/g
  ...
  private calculateCyclomaticComplexity(code: string, _language: string): number {
    ...
    const decisionPoints =
      code.match(/\b(if|else if|for|while|switch|case|catch|try|\?|&&|\|\|)\b/g) || []
  ...
  private calculateCommentQualityScore(code: string, language: string): number {
    ...
    for (const comment of commentLines) {
      ...
      if (comment.match(/\b(param|return|throws|author|since)\b/i)) {
  ...
  private calculateTestQualityScore(code: string, _language: string): number {
    ...
    return testFunctions.length > 0
      ? Math.round((assertions.length / testFunctions.length) * 100)
      : 0
  }
  ...
  private calculateResourceManagementScore(code: string, _language: string): number {
    ...
    const fileOperations = (code.match(/\b(new\s+)?File(Input|Output|Reader|Writer)\b/g) || [])
      .length
