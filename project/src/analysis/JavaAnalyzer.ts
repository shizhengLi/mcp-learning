...
export class JavaAnalyzer extends BaseLanguageAnalyzer {
  ...
  protected analyzeCodeStructure(code: string): {
    ...
    const functions = this.extractFunctions(
      code,
      /(?:public|private|protected|static|final)\s+\w+\s+(\w+)\s*\([^)]*\)\s*(?:throws\s+[\w\s,]+)?\s*\{/g
    )
    const classes = this.extractClasses(
      code,
      /(?:public|private|protected|static|final|abstract)?\s*(?:class|interface|enum)\s+(\w+)/g
    )
  ...
  protected generateLanguageSpecificSuggestions(
    ...
    const stringConcatenationInLoops = lines.filter((line, index) => {
      return (
        ...
        lines
          ...
          .some(l => l.includes('for') || l.includes('while'))
      )
    }).length
    ...
    const manualResourceManagement = lines.filter(
      line =>
        line.includes('new ') &&
        (line.includes('FileInputStream') ||
          line.includes('FileOutputStream') ||
    ...
    const traditionalLoops = lines.filter(
      line => line.includes('for (int i = 0; i <') || line.includes('for (String item :')
    ).length
    ...
    const nullChecks = lines.filter(
      line => line.includes('== null') || line.includes('!= null')
    ).length
