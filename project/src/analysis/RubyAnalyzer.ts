  ...
  private checkMetaprogramming(_content: string, _lines: string[]): any[] {
    ...
    if (defineMethodCount > 5) {
      issues.push({
        ...
        line: 1,
        suggestion:
          'Consider if metaprogramming is necessary or if regular methods would be clearer',
      })
    }
  ...
  private generateSuggestions(
    ...
    if (metrics.maintainability < 70) {
      suggestions.push({
        ...
        priority: 'medium',
        description:
          'Improve code maintainability by adding more comments and reducing method length',
        line: 1,
        estimatedImpact: {
