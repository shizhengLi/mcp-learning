import { BaseCodeAnalyzer, AnalysisResult, AnalysisOptions, CodeMetrics, AnalysisIssue, RefactoringSuggestion } from './BaseCodeAnalyzer';

export class GoAnalyzer extends BaseCodeAnalyzer {
  private readonly complexityKeywords = [
    'if', 'else', 'for', 'switch', 'select', 'case', 'default', 'func', 'defer', 'go'
  ];
  ...
  private async analyzeGoCode(code: string, filePath: string, options: AnalysisOptions): Promise<AnalysisResult> {
