...
export class PythonAnalyzer extends BaseLanguageAnalyzer {
  ...
  public calculatePythonMetrics(code: string, complexity: number): CodeMetrics {
    ...
    const maintainability = this.calculateMaintainability({
      ...
      averageFunctionLength: structure.functions.length > 0 
        ? lines.length / structure.functions.length 
        : 0,
  ...
  public generatePythonSuggestions(code: string, _lines: string[], metrics: CodeMetrics): RefactoringSuggestion[] {
