...
export class TechnicalDebtAnalyzer {
  private readonly interestRates = {
    ...
    performance_issue: 0.10 // 10% monthly interest
  };
  ...
  private identifyCodeSmells(
    ...
    lines.forEach((line, index) => {
      ...
      // Magic numbers
      if (trimmed.match(/\b[2-9]\d{2,}\b/) && !trimmed.includes('port') && !trimmed.includes('http')) {
        items.push(this.createDebtItem(
          'code_smell',
          'low',
  ...
  private identifyBugs(
    ...
    lines.forEach((line, index) => {
      ...
      // Resource leaks
      if (trimmed.match(/\b(new\s+\w+|open|connect)\b/) && !trimmed.includes('close') && !trimmed.includes('dispose')) {
        items.push(this.createDebtItem(
          'bug',
          'high',
  ...
  private getDefaultFixTime(type: TechnicalDebtItem['type'], severity: TechnicalDebtItem['severity']): number {
    const baseTimes = {
      code_smell: 2,
      design_issue: 4,
      bug: 3,
  ...
  private calculatePrincipal(type: TechnicalDebtItem['type'], severity: TechnicalDebtItem['severity']): number {
    const basePrincipal = {
      code_smell: 50,
      design_issue: 100,
      bug: 150,
  ...
  private calculateImpact(type: TechnicalDebtItem['type'], severity: TechnicalDebtItem['severity']) {
    const baseImpact = {
  ...
  private calculatePriority(type: TechnicalDebtItem['type'], severity: TechnicalDebtItem['severity']): number {
    const typePriority = {
      code_smell: 1,
      design_issue: 2,
      bug: 3,
  ...
  private generateDebtRecommendations(
    ...
    if (performanceIssues.length > 0) {
      recommendations.push({
        ...
        estimatedEffort: performanceIssues.reduce((total, item) => total + item.estimatedFixTime, 0),
  ...
  private assessDebtRisk(debtItems: TechnicalDebtItem[], _qualityMetrics: QualityMetrics): {
    ...
    const maintainabilityRisk = debtItems.some(item => item.impact.maintainability > 10) ? 'high' : 
                               debtItems.some(item => item.impact.maintainability > 5) ? 'medium' : 'low';
    
    const performanceRisk = debtItems.some(item => item.impact.performance > 10) ? 'high' :
                           debtItems.some(item => item.impact.performance > 5) ? 'medium' : 'low';
    
    const securityRisk = vulnerabilityCount > 1 ? 'high' : vulnerabilityCount > 0 ? 'medium' : 'low';
    
    const reliabilityRisk = debtItems.some(item => item.impact.reliability > 10) ? 'high' :
                           debtItems.some(item => item.impact.reliability > 5) ? 'medium' : 'low';
  ...
  private extractFunctions(code: string, language: string): Array<{ name: string; line: number; complexity: number }> {
    ...
    const pattern = functionPattern[language as keyof typeof functionPattern] || /\bfunction\s+(\w+)\b/g;
    let match;
  ...
  private estimateFunctionComplexity(code: string, startIndex: number): number {
    ...
    const complexityKeywords = functionCode.match(/\b(if|else if|for|while|switch|case|catch|try)\b/g) || [];
