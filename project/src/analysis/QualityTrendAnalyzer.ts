...
export class QualityTrendAnalyzer {
  ...
  analyzeTrends(
    ...
    const relevantData = sortedData.slice(0, Math.max(2, Math.min(period, sortedData.length))).reverse();
  ...
  private calculateRSquared(xValues: number[], yValues: number[], slope: number, intercept: number): number {
  ...
  private generateInsights(
    ...
    if (Math.abs(qualityChange) > this.trendThresholds.significantChange) {
      keyFindings.push(`Quality score is ${qualityTrend} with ${Math.abs(qualityChange).toFixed(1)}% change`);
    } else {
      keyFindings.push(`Quality score is ${qualityTrend} (${Math.abs(qualityChange).toFixed(1)}% change)`);
    }
    ...
    else if (qualityTrend === 'declining' || metrics.qualityScore.volatility > 20) riskAssessment = 'medium';
