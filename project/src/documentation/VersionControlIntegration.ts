...
export class VersionControlIntegration {
  ...
  async revertDocumentationVersion(projectPath: string, version: string): Promise<boolean> {
    try {
      await this.initialize()
      ...
      await this.saveDocumentationVersion(projectPath, snapshot, `Reverted to version ${version}`)
  ...
  async createBranch(branchName: string, fromVersion?: string): Promise<void> {
    try {
      ...
      if (snapshot) {
        await fs.writeFile(path.join(branchDir, 'snapshot.json'), JSON.stringify(snapshot, null, 2))
      }
      ...
      await fs.writeFile(path.join(branchDir, 'branch.json'), JSON.stringify(branchInfo, null, 2))
  ...
  async mergeBranch(branchName: string, message: string): Promise<void> {
    try {
      ...
      await this.saveDocumentationVersion('', _snapshot, `Merged branch ${branchName}: ${message}`)
