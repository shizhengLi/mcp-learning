...
export class DocumentationGenerator {
  ...
  async generateProjectDocumentation(
    ...
    if (includeSource) {
      documentation.code = await this.generateCodeDocumentation(projectPath, formats[0] || 'markdown');
    }
  ...
  private categorizeFileType(file: string, extension: string): 'source' | 'test' | 'config' | 'documentation' | 'asset' {
    if (file.includes('/test/') || file.includes('/tests/') || file.includes('.test.') || file.includes('.spec.')) {
      return 'test';
  ...
  private async generateSingleFileDocumentation(filePath: string): Promise<CodeDocumentation | null> {
    try {
  ...
  private async getSourceFiles(dirPath: string, includeTests: boolean): Promise<string[]> {
    ...
    return files.filter(file => {
      const extension = path.extname(file);
      const isTestFile = file.includes('/test/') || file.includes('.test.') || file.includes('.spec.');
      
      return ['.js', '.ts', '.jsx', '.tsx', '.py', '.java', '.go', '.rs', '.rb', '.php'].includes(extension) &&
  ...
  private async extractTestingInfo(projectPath: string): Promise<any> {
    const packageJson = await this.readPackageJson(path.join(projectPath, 'package.json'));
    const testDependencies = Object.keys(packageJson.devDependencies || {})
      .filter(dep => dep.toLowerCase().includes('test') || dep.toLowerCase().includes('jest') || dep.toLowerCase().includes('mocha'));

    return {
      frameworks: testDependencies,
      commands: packageJson.scripts ? Object.keys(packageJson.scripts).filter(key => key.includes('test')) : [],
  ...
  private inferType(filename: string): string {
    if (filename.toLowerCase().includes('api') || filename.toLowerCase().includes('endpoint')) {
      return 'api';
    } else if (filename.toLowerCase().includes('diagram') || filename.toLowerCase().includes('arch')) {
      return 'diagram';
    } else if (filename.toLowerCase().includes('code') || filename.toLowerCase().includes('source')) {
      return 'code';
    } else {
      return 'project';
    }
