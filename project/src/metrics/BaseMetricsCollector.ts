...
export class BaseMetricsCollector implements MetricsCollector {
  ...
  recordRequest(metrics: RequestMetrics): void {
    ...
    this.incrementCounter('requests_duration', metrics.duration, { method: metrics.method, path: metrics.path });
    
    if (metrics.success) {
      this.incrementCounter('requests_successful', 1, { method: metrics.method, path: metrics.path });
    } else {
  ...
  getAggregatedMetrics(timeRange: number = 3600000): Record<string, any> {
    ...
    return {
      ...
      performance: {
        avgMemoryUsage: filteredPerformance.length > 0
          ? filteredPerformance.reduce((sum, perf) => sum + perf.memoryUsage.percentage, 0) / filteredPerformance.length
          : 0,
        avgCpuUsage: filteredPerformance.length > 0
          ? filteredPerformance.reduce((sum, perf) => sum + perf.cpuUsage, 0) / filteredPerformance.length
          : 0,
      },
      timeRange,
      timestamp: now,
