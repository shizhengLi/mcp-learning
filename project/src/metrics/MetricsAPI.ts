...
export class MetricsAPI {
  ...
  healthCheck(req: Request, res: Response): void {
    try {
      ...
      if (health.memoryUsage > 90 || health.errorRate > 0.1 || alerts.some(a => a.level === 'critical')) {
        health.status = 'critical';
      } else if (health.memoryUsage > 80 || health.errorRate > 0.05 || alerts.some(a => a.level === 'warning')) {
