...
export class LocalModel implements AIModel {
  ...
  async generate(prompt: string, _context?: AIContext): Promise<AIResponse> {
    try {
      ...
      throw new Error(
        `Local model API error: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  private isSuggestionLine(line: string): boolean {
    return (
      line.match(/(suggest|recommend|improve|optimize|refactor|restructure)/i) !== null ||
  ...
  private calculateConfidence(response: OllamaResponse): number {
    ...
    if (response.total_duration && response.total_duration < 10000000000) {
      // 10 seconds
      confidence += 0.05
    }
