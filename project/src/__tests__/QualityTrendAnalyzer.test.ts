...
function createTestTrendDataPoint(
  ...
  const metrics: QualityMetrics = {
    ...
    qualityGrade:
      qualityScore >= 90 ? 'A' : qualityScore >= 80 ? 'B' : qualityScore >= 70 ? 'C' : 'D',
    qualityTrend: 'stable',
    lineLength: {
      average: 45,
      max: 80,
...
describe('QualityTrendAnalyzer', () => {
  ...
  describe('Basic Trend Analysis', () => {
    ...
    it('should throw error with insufficient data', () => {
      const historicalData: TrendDataPoint[] = [createTestTrendDataPoint(new Date(), 80)]
  ...
  describe('Insights Generation', () => {
    ...
    it('should identify high volatility issues', () => {
      ...
      const volatileScores = [
        20, 95, 30, 85, 25, 90, 35, 80, 40, 75, 45, 70, 50, 65, 55, 60, 45, 70, 40, 75, 35, 65, 30,
        70, 25, 75, 20, 80, 15, 85,
      ]
      for (let i = 0; i < 30; i++) {
        const date = new Date(baseDate.getTime() - (29 - i) * 24 * 60 * 60 * 1000)
  ...
  describe('Trend Comparison', () => {
    it('should compare two trend analyses', () => {
      ...
      const comparison = trendAnalyzer.compareTrends(currentAnalysis, previousAnalysis, {
        industryAverage: 72,
        teamAverage: 75,
        projectTarget: 80,
      })
  ...
  describe('Data Export/Import', () => {
    ...
    it('should import trend data from JSON', () => {
      ...
      expect(importedAnalysis.metrics.qualityScore.trend).toBe(
        originalAnalysis.metrics.qualityScore.trend
      )
