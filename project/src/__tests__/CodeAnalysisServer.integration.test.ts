...
describe('CodeAnalysisServer Integration Tests', () => {
  ...
  describe('Server Process Integration', () => {
    it(
      'should start and stop server process correctly',
      async () => {
...
`
  ...
  describe('End-to-End Code Analysis Workflow', () => {
    it(
      'should analyze Python code and return structured results',
      async () => {
...
if __name__ == "__main__":
    ...
    it(
      'should analyze JavaScript code and return structured results',
      async () => {
        server = new CodeAnalysisServer(false)
...
def invalid_function(
  ...
  describe('Multi-File Analysis Integration', () => {
    it(
      'should analyze multiple files and return aggregated results',
      async () => {
        server = new CodeAnalysisServer(false)
...
`
        ...
        for (const filePath of testFiles) {
          ...
          if (language) {
            ...
            const result = await (server as any).analyzeInMemoryCode(analyzer, mockCode, filePath, {
              includeSuggestions: true,
            })
            results.push(result)
          }
  ...
  describe('Refactoring Suggestions Integration', () => {
    it(
      'should generate refactoring suggestions for complex code',
      async () => {
        server = new CodeAnalysisServer(false)
...
def calculate_metrics(values):
    ...
    it(
      'should handle refactoring suggestions with context',
      async () => {
        server = new CodeAnalysisServer(false)
...
`
  ...
  describe('Language Detection Integration', () => {
    it('should correctly detect Python files from various extensions', () => {
      ...
      const pythonFiles = ['test.py', 'main.py', 'script.py', '/path/to/file.py']
    ...
    it('should correctly detect JavaScript files from various extensions', () => {
      ...
      const jsFiles = ['test.js', 'app.jsx', '/path/to/script.js']

      const tsFiles = ['component.ts', 'page.tsx']
    ...
    it('should return null for unsupported file types', () => {
      ...
      const unsupportedFiles = ['test.xml', 'config.json', 'style.css', 'unknown.xyz']

      const supportedFiles = ['test.java', 'file.cpp', 'script.rb', 'code.go']
    ...
    describe('Performance Metrics Integration', () => {
      it(
        'should calculate performance metrics for code analysis',
        async () => {
          server = new CodeAnalysisServer(false)
      ...
      it(
        ...
        async () => {
          ...
          const loadTestRequests = Array(20)
            .fill(null)
            .map((_, i) =>
              (server as any).analyzeInMemoryCode(
                (server as any).pythonAnalyzer,
