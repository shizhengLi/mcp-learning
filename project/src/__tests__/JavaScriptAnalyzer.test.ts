...
import { helper } from '/helpers/helper';`
  ...
  describe('JavaScript Style Checking', () => {
    it('should detect long lines', () => {
      ...
      const issues = (analyzer as any).checkLanguageSpecificRules(longLine, [
        'MISSING_SEMICOLONS',
        'NO_VAR_USAGE',
        'STRICT_EQUALITY',
      ])
    ...
    it('should detect missing semicolons', () => {
      ...
      const issues = (analyzer as any).checkLanguageSpecificRules(code, [
        'MISSING_SEMICOLONS',
        'NO_VAR_USAGE',
        'STRICT_EQUALITY',
      ])
      ...
      expect(
        issues.some((issue: AnalysisIssue) => issue.message.includes('Missing semicolon'))
      ).toBe(true)
    })
    ...
    it('should detect var usage', () => {
      ...
      expect(
        issues.some((issue: AnalysisIssue) => issue.message.includes('let or const instead of var'))
      ).toBe(true)
    })
    ...
    it('should detect double equals', () => {
      ...
      expect(
        issues.some((issue: AnalysisIssue) => issue.message.includes('=== instead of =='))
      ).toBe(true)
    })
  ...
  describe('JavaScript Best Practices Checking', () => {
    it('should detect console statements', () => {
      ...
      expect(
        issues.some((issue: AnalysisIssue) => issue.message.includes('Console statement'))
      ).toBe(true)
    })
    ...
    it('should detect eval usage', () => {
      ...
      expect(
        issues.some((issue: AnalysisIssue) => issue.message.includes('eval() usage is dangerous'))
      ).toBe(true)
  ...
  .catch(error => handleError(error));`
      ...
      expect(
        suggestions.some((s: RefactoringSuggestion) => s.description.includes('async/await'))
      ).toBe(true)
    })
  ...
  return item * 2;
      ...
      expect(
        suggestions.some((s: RefactoringSuggestion) => s.description.includes('arrow functions'))
      ).toBe(true)
    })
...
const email = user.email;`
  ...
  try {
    ...
    return [];
      ...
      expect(
        result.issues.some((issue: AnalysisIssue) =>
          issue.message.includes('let or const instead of var')
        )
      ).toBe(true)
      expect(
        result.issues.some((issue: AnalysisIssue) => issue.message.includes('Console statement'))
      ).toBe(true)
