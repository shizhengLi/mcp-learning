...
describe('AIModelManager Tests', () => {
  ...
  describe('Refactoring Suggestions', () => {
    beforeEach(() => {
      ...
      mockClaudeModel.generate.mockResolvedValue({
        content: '{"suggestions": [{"type": "optimize", "priority": "high", "description": "Test suggestion"}]}',
        suggestions: [{
          type: 'optimize',
          priority: 'high',
  ...
  describe('Code Analysis', () => {
    beforeEach(() => {
      mockClaudeModel.isAvailable.mockResolvedValue(true);
      mockClaudeModel.generate.mockResolvedValue({
        content: '{"complexity": 5, "maintainability": 85, "issues": [], "insights": "Clean code structure"}',
        suggestions: [],
        confidence: 0.9,
        model: 'claude'
      });
    ...
    it('should parse AI analysis response correctly', async () => {
      mockClaudeModel.generate.mockResolvedValue({
        content: '{"complexity": 7, "maintainability": 75, "issues": [{"type": "warning", "severity": "medium", "message": "Consider adding comments", "line": 1, "rule": "COMMENT-001"}], "insights": "Good overall structure"}',
        suggestions: [],
        confidence: 0.9,
        model: 'claude'
      });
