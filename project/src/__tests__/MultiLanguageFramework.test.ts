...
def complex_function(data):
  ...
  describe('Multiple File Analysis', () => {
    it('should analyze multiple files of different languages', async () => {
      const files = ['test.py', 'script.js', 'component.ts', 'Main.java']
  ...
  describe('Error Handling', () => {
    it('should throw error for unsupported file types', async () => {
      await expect(framework.analyzeFile('test.unknown')).rejects.toThrow(
        'Unsupported file type: test.unknown'
      )
    })

    it('should throw error for unsupported languages', async () => {
      await expect(framework.analyzeCode('code', 'unknown')).rejects.toThrow(
        'No configuration found for language: unknown'
      )
    })

    it('should throw error when analyzer is not available', async () => {
      await expect(framework.analyzeCode('code', 'cpp')).rejects.toThrow(
        'No analyzer available for language: cpp'
      )
    })
