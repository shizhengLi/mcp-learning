        ...
        try {
          ...
          if (analysisOptions.includeSuggestions) {
            try {
              ...
              const aiSuggestions = await this.aiModelManager.generateRefactoringSuggestions(
                code,
                aiContext
              )
    ...
    this.server.tool(
      ...
      async ({ filePath, options }) => {
        try {
          ...
          const result = await this.analyzeInMemoryCode(
            analyzer,
            mockCode,
            filePath,
            analysisOptions
    ...
    this.server.tool(
      ...
      async ({ code, language, context }) => {
        try {
          ...
          const formattedSuggestions = this.formatRefactoringSuggestions(
            result.suggestions,
            context
          )
    ...
    this.server.tool(
      ...
      async ({ code, language, filePath, options }) => {
        try {
          ...
          const result = await this.multiLanguageFramework.analyzeCode(
            code,
            language,
            filePath,
            analysisOptions
  ...
  private formatAIAnalysisResult(
    ...
    if (traditionalResult && options.traditional && aiInsights && options.ai) {
      ...
      const totalSuggestions =
        (traditionalResult.suggestions.length || 0) + (aiInsights.suggestions?.length || 0)
